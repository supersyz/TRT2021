...
class PanopticFCN(nn.Module):
    ...
    def inference_single_level(self, pred_center, pred_region, pred_weights, pool_size):
        ...
        # stuff_num = keep.sum()
        stuff_num = score_st.shape[0]
        print('stuff num: ', stuff_num)
    ...
    def inference(self, batch_inputs, images, pred_centers, pred_regions, pred_weights, encode_feat):
        ...
        for img_idx, result_img in enumerate(results):
            ...
            print('check if it can comback for mask size?')
            # for i in class_ths:
            #     print(i)
            #     print('class_ths: ', i.shape)
    ...
    @torch.no_grad()
    def inference_single_level_onnx(self, pred_center, pred_region, pred_weights, pool_size, level_idx):
        # pred things
        pred_center = pred_center.sigmoid()
        center_pool = F.avg_pool2d(pred_center, kernel_size=pool_size,
                                   stride=1, padding=(pool_size-1)//2)
        ...
        top_num = center_shape[-2]*center_shape[-1]
        if level_idx < 4:
            top_num = self.center_top_num // 2
        #if top_num == 50:
        print(pred_center.shape, top_num)
        ...
        inst_w = pred_weights.reshape(*weight_shape[:2], -1)
        idx_inst = index.unsqueeze(1).expand(*weight_shape[:2], -1)
        print('inst_w: ', inst_w.shape, 'idx_inst: ', idx_inst.shape)
        from .onnx_patch import gather
        # idx_feat_th = torch.gather(inst_w, dim=2, index=idx_inst)
        idx_feat_th = gather(inst_w, dim=2, index=idx_inst)
        print('idx_feat_th: ', idx_feat_th.shape)
        idx_feat_th = idx_feat_th.unsqueeze(-1)
        ...
        # return idx_feat_th, class_th, score_th, thing_num, idx_feat_st, score_st, class_st, stuff_num
        return pred_center, pred_region, pred_cate, pred_st_mask, score_st, idx_feat_st, class_th, score_th, idx_feat_th

    @torch.no_grad()
    def inference_onnx(self, batch_inputs, images, pred_centers, pred_regions, pred_weights, encode_feat):
        """
        Panoptic FCN inference process.
        ...
        print('imgshape: ', img_shape, batch_inputs.shape)
        results = batch_inputs
        processed_results = []
        # we only consider 1 batch situation
        assert batch_inputs.shape[0] == 1, 'only batchsize == 1 supported.'

        # print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
        # print(encode_feat.shape,encode_feat[0].shape)
        # a,b,c = encode_feat[0].shape[0],encode_feat[0].shape[1],encode_feat[0].shape[2]
        # #encode_feat = encode_feat[0].unsqueeze(0)
        # encode_feat = encode_feat[0].reshape(1,a,b,c)
        encode_feat = encode_feat[0].unsqueeze(0)
        feat_shape = encode_feat.shape
        encode_feat = encode_feat.reshape(*feat_shape[:2], -1)
        print('encode feat: {}, feat_shape: {}'.format(encode_feat, feat_shape))
        # result_instance = None

        pred_center_0, pred_region_0, pred_cate_0, pred_st_mask_0, score_st_0, idx_feat_st_0, class_th_0, score_th_0, idx_feat_th_0 = \
            self.inference_single_level_onnx(
                pred_centers[0], pred_regions[0], pred_weights[0], 3, 0)
        pred_center_1, pred_region_1, pred_cate_1, pred_st_mask_1, score_st_1, idx_feat_st_1, class_th_1, score_th_1, idx_feat_th_1 = \
            self.inference_single_level_onnx(
                pred_centers[1], pred_regions[1], pred_weights[1], 3, 1)
        pred_center_2, pred_region_2, pred_cate_2, pred_st_mask_2, score_st_2, idx_feat_st_2, class_th_2, score_th_2, idx_feat_th_2 = \
            self.inference_single_level_onnx(
                pred_centers[2], pred_regions[2], pred_weights[2], 3, 2)
        pred_center_3, pred_region_3, pred_cate_3, pred_st_mask_3, score_st_3, idx_feat_st_3, class_th_3, score_th_3, idx_feat_th_3 = \
            self.inference_single_level_onnx(
                pred_centers[3], pred_regions[3], pred_weights[3], 5, 3)
        pred_center_4, pred_region_4, pred_cate_4, pred_st_mask_4, score_st_4, idx_feat_st_4, class_th_4, score_th_4, idx_feat_th_4 = \
            self.inference_single_level_onnx(
                pred_centers[4], pred_regions[4], pred_weights[4], 5, 4)

        print('OK, we pased inference_single_level_onnx..')
        # return pred_center_0, pred_region_0, pred_cate_0, pred_st_mask_0, score_st_0, idx_feat_st_0, \
        #     pred_center_1, pred_region_1, pred_cate_1, pred_st_mask_1, score_st_1, idx_feat_st_1, \
        #     pred_center_2, pred_region_2, pred_cate_2, pred_st_mask_2, score_st_2, idx_feat_st_2, \
        #     pred_center_3, pred_region_3, pred_cate_3, pred_st_mask_3, score_st_3, idx_feat_st_3, \
        #     pred_center_4, pred_region_4, pred_cate_4, pred_st_mask_4, score_st_4, idx_feat_st_4, \
        #     encode_feat
        ...
        class_ths = torch.cat(
            [class_th_0, class_th_1, class_th_2, class_th_3, class_th_4], dim=1).squeeze(0)
        score_ths = torch.cat(
            [score_th_0, score_th_1, score_th_2, score_th_3, score_th_4], dim=1).squeeze(0)
        idx_feat_th = torch.cat(
            [idx_feat_th_0, idx_feat_th_1, idx_feat_th_2, idx_feat_th_3, idx_feat_th_4], dim=2)
        print('class_ths: ', class_ths.shape)
        print('score_ths: ', score_ths.shape)
        print('idx_feat_th: ', idx_feat_th.shape)
        thing_num = class_ths.shape[0]
        print('things num: ', thing_num)
        # stuff_num = sum(stuff_num)
        # if stuff_num == 0:
        #     class_sts, idx_feat_st, score_sts = [], [], []
        # else:
        ...
        pred_thing, class_ths, score_ths = self.thing_generator(
            encode_feat, feat_shape, idx_feat_th, thing_num, class_ths, score_ths)
        print('thing generator is done.', pred_thing.shape)

        # print(pred_thing.shape)
        ...
        # result_instance, pred_mask, class_ths, score_ths = self.process_inst(
        #     class_ths, score_ths, pred_thing, batch_inputs.shape[2:], [720, 1280])
        # processed_results.append({"instances": result_instance})

        return self.process_inst_onnx(class_ths, score_ths, pred_thing, img_shape, False)
