...
class SingleHead(nn.Module):
    ...
    def __init__(self, cfg, in_channel, conv_dims, num_convs, deform=False, coord=False, norm='', name=''):
        super().__init__()
        self.coord = coord
    ...
    def coord_conv(self, feat):
        with torch.no_grad():
            if self.export_onnx:
                ...
                xsp, ysp = torch.tensor(feat.shape[-2:]).to(torch.float32)
                x_pos = torch.arange(-1, 1, 2./xsp, device=feat.device)
                y_pos = torch.arange(-1, 1, 2./ysp, device=feat.device)
            else:
                x_pos = torch.linspace(-1, 1,
                                       feat.shape[-2], device=feat.device)
                y_pos = torch.linspace(-1, 1,
                                       feat.shape[-1], device=feat.device)

            grid_x, grid_y = torch.meshgrid(x_pos, y_pos)
            # grid_x = grid_x.unsqueeze(dim=0)
            # grid_x =grid_x.unsqueeze(dim=1)
            # grid_x = grid_x.expand(1, -1, -1, -1)
            grid_x = grid_x.unsqueeze(dim=0).unsqueeze(
                dim=1).expand(feat.shape[0], -1, -1, -1)

            # grid_y = grid_y.unsqueeze(dim=0)
            # grid_y = grid_y.unsqueeze(dim=1)
            # grid_y = grid_y.expand(1, -1, -1, -1)

            # a,b = grid_x.shape[0],grid_x.shape[1]
            # # grid_x = grid_x.unsqueeze(dim=0)
            # # grid_x =grid_x.unsqueeze(dim=1)
            # grid_x = grid_x.reshape(1,1,a,b)
            #
            # grid_x = grid_x.expand(1, -1, -1, -1)
            grid_y = grid_y.unsqueeze(dim=0).unsqueeze(
                dim=1).expand(feat.shape[0], -1, -1, -1)
            #
            # # grid_y = grid_y.unsqueeze(dim=0)
            # # grid_y = grid_y.unsqueeze(dim=1)
            # grid_y = grid_y.reshape(1, 1, a, b)
            # grid_y = grid_y.expand(1, -1, -1, -1)
...
class PositionHead(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.position_head = SingleHead(cfg, in_channel+2 if coord else in_channel,
                                        conv_dims,
                                        num_convs,
                                        deform=deform,
                                        coord=coord,
...
class FeatureEncoder(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.encode_head = SingleHead(cfg, in_channel+2 if coord else in_channel,
                                      conv_dims,
                                      num_convs,
                                      deform=deform,
                                      coord=coord,
...
class ThingGenerator(nn.Module):
    ...
    def forward(self, x, feat_shape, idx_feat, idx_shape, pred_cate=None, pred_score=None):
        if self.export_onnx:
            return self.forward_onnx(x, feat_shape, idx_feat, idx_shape, pred_cate, pred_score)
        else:
            n, c, h, w = feat_shape
            if idx_shape > 0:
                ...
                # seems not kernel fusion leads to duplicated results
    ...
    def forward_onnx(self, x, feat_shape, idx_feat, idx_shape, pred_cate=None, pred_score=None):
        n, c, h, w = feat_shape
        if idx_shape > 0:
            meta_weight = self.embed_extractor(idx_feat)
            meta_weight = meta_weight.squeeze(dim=-1).permute(0, 2, 1)
            # seems not kernel fusion leads to duplicated results
            # if not self.training:
            # meta_weight, pred_cate, pred_score = self.kernel_fusion(meta_weight, pred_cate, pred_score)
            inst_pred = torch.matmul(meta_weight, x)
