...
ICudaEngine *BuildEngineProc(IBuilder *builder, void *pData) {
    ...
    //创建插件
    SeafoodPlugin seafoodPlugin;
    std::cout<<"[PLUGIN] Logs_Seafood: plugin Seafood create success"<<std::endl;
    ITensor *aInputTensor[] = {tensor};
    tensor = network->addPluginV2(aInputTensor, 1, seafoodPlugin)->getOutput(0);
    if (pParam->bInt8) {
    ...
    std::cout<<"[Engine] Logs_Seafood: Engine with Plugin Create success"<<std::endl;
    config->destroy();
    network->destroy();
...
int main(int argc, char** argv) {
    ...
    BuildEngineParamExt param = {{1, 1, 1, 256}, bExplitBatch ? nBatch : 0};
    param.bInt8 = false;
    param.bFp16 = false;
